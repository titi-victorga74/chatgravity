...
router.post('/data', authenticateToken, (req, res) => {
    ...
    db.run(`INSERT INTO data_entries (user_id, content, area) VALUES (?, ?, ?)`,
...
router.get('/taquera-options', (req, res) => {
    db.all(`SELECT * FROM taquera_options`, [], (err, rows) => {
        if (err) return res.status(500).json({ error: err.message });
        console.log("Serving taquera options:", rows); // Debug log
        res.json(rows);
    });
});
...
router.get('/taquera-items', (req, res) => {
    ...
    // If cardId is missing, undefined, or null string, return empty list
    // This prevents showing ALL items when no card is selected or context is lost
...
router.post('/taquera-items', (req, res) => {
    ...
    db.run(`INSERT INTO taquera_items (name, quantity, card_id) VALUES (?, ?, ?)`,
...
router.get('/taquera-counts', (req, res) => {
    // This query counts items per card and status
    const query = `
    ...
    db.all(query, [], (err, rows) => {
        ...
        // Transform the flat list into a nested object: { cardId: { pending: 5, delivered: 2 } }
        const counts = {};
        rows.forEach(row => {
            ...
            // Map status to our keys if needed, assuming 'pending' and 'delivered' (or 'completed')
            if (statusKey === 'pending') {
                counts[row.card_id].pending = row.count;
            } else if (statusKey === 'delivered' || statusKey === 'entregado' || statusKey === 'completed') {
                counts[row.card_id].delivered = row.count; // or 'completed'
            }
        });
...
router.put('/taquera-items/:id', (req, res) => {
    ...
    db.run(`UPDATE taquera_items SET status = ? WHERE id = ?`, [status, id], function (err) {
        if (err) return res.status(500).json({ error: err.message });
...
router.delete('/taquera-items/:id', (req, res) => {
    const { id } = req.params;
    db.run(`DELETE FROM taquera_items WHERE id = ?`, [id], function (err) {
        if (err) return res.status(500).json({ error: err.message });
        res.json({ message: "Deleted successfully", changes: this.changes });
    });
});
...
router.get('/devoluciones-counts', (req, res) => {
    ...
    db.all(query, [], (err, rows) => {
        ...
        // Transform to object { cardId: count }
        const counts = {};
        rows.forEach(row => {
...
router.post('/devoluciones-items', (req, res) => {
    ...
    db.run(query, [cardId, name, quantity || 1], function (err) {
        if (err) return res.status(500).json({ error: err.message });
        res.json({ id: this.lastID });
    });
});
...
router.get('/card-items/:cardId', (req, res) => {
    ...
    db.all(`SELECT card_items.*, users.username FROM card_items 
        ...
        console.log(`[API] Found ${rows.length} items for cardId ${cardId}:`, JSON.stringify(rows));
        res.json(rows);
    });
});
...
router.post('/card-items', authenticateToken, (req, res) => {
    ...
    db.run(`INSERT INTO card_items (card_id, text, user_id) VALUES (?, ?, ?)`,
        ...
        function (err) {
            ...
            console.log(`[API] Item inserted successfully. New ID: ${this.lastID}`);
            res.json({ id: this.lastID, username: req.user.username });
        }
    );
});
...
router.delete('/card-items/:id', authenticateToken, (req, res) => {
    const { id } = req.params;
    // Allow deleting if user is admin or the owner (for simplicity, just admin or any auth user for now to unblock)
    // Ideally check ownership.
    db.run(`DELETE FROM card_items WHERE id = ?`, [id], function (err) {
        if (err) return res.status(500).json({ error: err.message });
